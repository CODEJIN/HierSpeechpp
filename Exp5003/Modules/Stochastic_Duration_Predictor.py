...
class Stochastic_Duration_Predictor(torch.nn.Module):
    def __init__(
        ...
        self.prenet_pre = torch.nn.Conv1d(
        ...
        self.prenet_post = torch.nn.Conv1d(
            in_channels= calc_channels,
            out_channels= calc_channels,
            kernel_size= 1
            )

        self.postnet_pre = torch.nn.Conv1d(
            in_channels= 1,
            out_channels= calc_channels,
            kernel_size= 1,
            )
        ...
        self.postnet_post = torch.nn.Conv1d(
            in_channels= calc_channels,
            out_channels= calc_channels,
            kernel_size= 1
            )
    ...
    def forward(
        ...
        if not reverse:
            ...
            durations = (z[:, 0].exp() * masks.squeeze(1)).clamp(0.0).ceil().long()
